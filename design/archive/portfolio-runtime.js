...
/** Mount the portfolio enhancements after hydration. Returns an idempotent disposer. */
export function initializePortfolio() {
  "use strict";
  const root = document.documentElement;
  const originalScrollRestoration = history.scrollRestoration;
  history.scrollRestoration = "manual";
  // Attribute snapshots let Strict Mode and Fast Refresh return to the server-rendered state.
  const ownedElements = [
    root,
    document.body,
  ...
  const originalAttributes = ownedElements.map((element) => [
    element,
  ...
  const originalContents = [
    ...document.querySelectorAll(
      ".hero-word, .section h2:not(.sr-only), .stage h3, .stage-count, .skill-status",
    ),
  ].map((element) => [element, [...element.childNodes]]);
  ...
  let layoutSize = null;
  const stableHeight = () =>
    probe.getBoundingClientRect().height || innerHeight;
  const visibleHeight = () => {
  ...
  const motionValue = (name, fallback) =>
    parseFloat(motionStyle.getPropertyValue(name)) || fallback;
  const motion = {
    enter: motionValue("--enter", 320),
    control: motionValue("--control", 200),
    stagger: motionValue("--stagger", 40),
    entryOffset: motionValue("--entry-offset", 12),
    skillsOffset: motionValue("--skills-offset", 8),
    stateOffset: motionValue("--state-offset", 8),
    easeOut:
      motionStyle.getPropertyValue("--ease-out").trim() ||
  ...
  function animateMotion(
    ...
    try {
      ...
      /* Unsupported animation options leave the readable base state in place. */
    }
  }
  function enterHero() {
    ...
    if (
      ...
      return;
    const targets = [
      document.querySelector(".hero-identity"),
      document.querySelector(".hero-scene"),
      document.querySelector(".hero-description"),
      document.querySelector(".hero-side .text-link"),
    ];
    targets.forEach((element, index) =>
      animateMotion(
        ...
        [
          {
            opacity: 0.85,
            transform: "translateY(" + motion.entryOffset + "px)",
          },
          { opacity: 1, transform: "translateY(0)" },
        ],
        heroMotion,
        motion.enter,
        Math.min(index, 2) * motion.stagger,
      ),
    );
  }
  function stopWork() {
  ...
  const stories = [...document.querySelectorAll("[data-story]")].map(
    (track) => ({
      track,
      panel: track.querySelector(".story-panel"),
      top: track.querySelector(".story-top"),
      stack: track.querySelector(".stage-stack"),
      stages: [...track.querySelectorAll("[data-stage]")],
      buttons: [...track.querySelectorAll("[data-stage-button]")],
      count: track.querySelector(".stage-count"),
      line: track.querySelector(".progress-line"),
      marker: track.querySelector(".progress-marker"),
      pinned: false,
      active: -1,
      motion: new Set(),
      start: 0,
  ...
  const contactCards = [...document.querySelectorAll(".contact-link")].map(
    (node) => ({
      node,
      entered: false,
      motion: new Set(),
  ...
  const metricGroups = stories
    .flatMap((story) =>
      story.stages.map((stage) => ({
        story,
        stage,
        cards: [...stage.querySelectorAll(".kpi-card")],
        active: -1,
      })),
    )
  ...
  const arts = [...document.querySelectorAll("[data-art]")].map((art) => ({
    element: art,
    owner: art.closest("[data-scene-scope]"),
    story: storyByName.get(art.dataset.art),
    visible: true,
    x: 0,
    y: 0,
    units: [...art.querySelectorAll(".scroll-unit")].map((node, index) => ({
      node,
      pointer: node.querySelector(".pointer-unit"),
      index,
  ...
  const headings = [
    ...document.querySelectorAll(
      ".hero-word, .section h2:not(.sr-only), .stage h3",
    ),
  ].map((element) => {
  ...
  function focusTarget(target) {
    const added =
      ...
      !target.matches("a,button,summary,input,select,textarea");
    if (added) target.setAttribute("tabindex", "-1");
    target.focus({ preventScroll: true });
    if (added)
      listen(target, "blur", () => target.removeAttribute("tabindex"), {
        once: true,
      });
  }
  function currentAnchor() {
    const viewport = visibleHeight();
    const line = headerHeight + Math.max(0, viewport - headerHeight) * 0.36;
    const pinned = stories.find(
      (s) => s.pinned && scrollY >= s.start && scrollY <= s.start + s.travel,
    );
    if (pinned) {
      ...
      return {
        ...
        story: pinned,
        index: Math.max(0, pinned.active),
        fraction: (scrollY - pinned.start) / pinned.step,
      };
    }
    let best = null,
      distance = Infinity;
    const candidates = document.querySelectorAll(
      "[data-stage], [data-reading-anchor], .hero-identity, .contact-intro, .site-footer",
    );
    ...
    if (best) {
      ...
      if (story) {
        best.story = story;
        best.index = story.stages.indexOf(best.element);
      }
    }
    return best;
  }
  function selectStage(story, index) {
    ...
    story.count.textContent =
      ...
      " / " +
      String(story.stages.length).padStart(2, "0");
    story.active = index;
  }
  function showStage(story, index) {
    ...
    if (
      story.active === index &&
      story.stages.every((stage, i) => stage.hidden === (i !== index))
    )
      return;
    const previous = story.active;
    cancelMotion(story.motion);
    story.stages.forEach((stage, i) => {
    ...
    if (
      ...
      story.pinned &&
      scrollY >= story.start &&
      scrollY <= story.start + story.travel
    ) {
      ...
      animateMotion(
        ...
        [
          {
            opacity: 0.85,
            transform: "translateY(" + direction * motion.stateOffset + "px)",
          },
          { opacity: 1, transform: "translateY(0)" },
        ],
        story.motion,
  ...
  function unpin(story) {
    ...
    story.active = -1;
    story.toolsHeight = 0;
    story.track.classList.remove(
      "is-pinned",
      "is-measuring",
      "has-sticky-tools",
    );
    story.track.classList.add("is-flow");
    story.track.style.removeProperty("height");
    story.track.style.removeProperty("--panel-height");
  ...
  function layout(restore = true) {
    if (measuring || disposed || pageSuspended || document.hidden) return;
    measuring = true;
    ...
    stories.forEach((story) => {
      ...
      let maxPanel = 0,
        maxStage = 0;
      story.stages.forEach((stage, index) => {
        story.stages.forEach((node, i) => {
          node.hidden = i !== index;
        });
        maxStage = Math.max(maxStage, stage.getBoundingClientRect().height);
        maxPanel = Math.max(
          maxPanel,
          story.panel.getBoundingClientRect().height,
        );
      });
      story.track.classList.remove("is-measuring");
      if (maxPanel > panelHeight - 2) {
        unpin(story);
        return;
      }
      story.pinned = true;
      story.panelHeight = panelHeight;
      ...
      story.stack.style.setProperty(
        "--stage-height",
        Math.ceil(maxStage) + "px",
      );
    ...
    stories.forEach((story) => {
      if (!story.pinned) {
        ...
        const sticky = panelHeight >= 240 && toolsHeight <= panelHeight * 0.3;
        story.track.classList.toggle("has-sticky-tools", sticky);
        story.toolsHeight = sticky ? toolsHeight : 0;
        story.track.style.setProperty(
          "--story-tools-height",
          story.toolsHeight + "px",
        );
      }
      story.start =
        story.track.getBoundingClientRect().top + scrollY - headerHeight;
    });
    layoutSize = {
    ...
    } else if (Math.abs(scrollY - destination) > 0.5)
  ...
  function revealContacts(reads) {
    ...
    reads.forEach(({ card, rect }) => {
      if (rect.bottom <= headerHeight || rect.top >= visibleHeight() * 0.9)
        return;
      card.entered = true;
      // A link under a pointer or keyboard focus should remain a stationary target.
      if (
        card.node.contains(document.activeElement) ||
        (finePointer.matches && card.node.matches(":hover"))
      )
        return;
      animateMotion(
        card.node,
        ...
        card.motion,
        motion.control,
        Math.min(order++, 2) * motion.stagger,
      );
    });
  }
  function paint() {
    ...
    const viewport = visibleHeight();
    const readLine = headerHeight + Math.max(0, viewport - headerHeight) * 0.36;
    const stageRects = stories.map((story) =>
      story.pinned
        ? []
        : story.stages.map((stage) => stage.getBoundingClientRect()),
    );
    const sectionRects = sections.map((element) => ({
      element,
      rect: element.getBoundingClientRect(),
    }));
    const headingRects = headings.map((heading) => ({
      heading,
      rect: heading.element.getBoundingClientRect(),
    }));
    const artRects = arts
      .filter((art) => art.visible)
      .map((art) => ({
        art,
        rect: art.element.getBoundingClientRect(),
        owner: art.owner.getBoundingClientRect(),
      }));
    const barWidths = stories.map(
      (story) => story.line.getBoundingClientRect().width,
    );
    const contactReads = contactCards
      .filter((card) => !card.entered)
      .map((card) => ({ card, rect: card.node.getBoundingClientRect() }));
    const metricReads = metricGroups.map((group) => ({
      group,
      rects:
        enabled && !group.story.pinned
          ? group.cards.map((card) => card.getBoundingClientRect())
          : [],
    }));
    let active = "home";
    sectionRects.forEach(({ element, rect }) => {
      if (rect.top <= readLine) active = element.id;
    });
    ...
    stories.forEach((story, k) => {
      if (!story.pinned) {
        ...
        const storyLine =
          headerHeight +
          story.toolsHeight +
          Math.max(0, viewport - headerHeight - story.toolsHeight) * 0.3;
        let index = 0;
        rects.forEach((rect, i) => {
          if (rect.top <= storyLine) index = i;
        });
        const span =
          index < rects.length - 1
            ? rects[index + 1].top - rects[index].top
            : rects[index].height;
      ...
      story.phase =
        index +
        (index < story.stages.length - 1 ? smooth((local - 0.78) / 0.22) : 0);
      story.local = local;
      story.marker.style.transform =
        "translateX(" + Math.max(0, barWidths[k] - 8) * story.progress + "px)";
    });
    ...
    metricReads.forEach(({ group, rects }) => {
      ...
      if (enabled && !group.stage.hidden) {
        if (group.story.pinned) {
          const progress = clamp((group.story.local - 0.14) / 0.72);
          active = Math.min(
            group.cards.length - 1,
            Math.floor(progress * group.cards.length),
          );
        } else {
          let distance = Infinity;
          rects.forEach((rect, index) => {
            if (
              rect.bottom <= headerHeight + group.story.toolsHeight ||
              rect.top >= viewport
            )
              return;
            const d = Math.abs(rect.top + rect.height * 0.5 - readLine);
            if (d < distance) {
              distance = d;
              active = index;
            }
      ...
      if (active !== group.active) {
        ...
        group.cards.forEach((card, index) => {
          if (index === active) card.dataset.emphasis = "true";
          else card.removeAttribute("data-emphasis");
        });
      }
    });
    headingRects.forEach(({ heading, rect }) => {
      ...
      let progress = clamp(
        (viewport - rect.top) / (Math.max(1, viewport - headerHeight) * 0.65),
      );
      const story =
        heading.stage && stories.find((s) => s.stages.includes(heading.stage));
      if (story?.pinned) progress = clamp((story.local + 0.05) / 0.5);
      heading.fills.forEach((fill, index) => {
        const p = enabled
          ? clamp(
              progress * 1.5 -
                (index / Math.max(1, heading.fills.length)) * 0.35,
            )
          : 1;
        fill.style.opacity = String(p);
      });
    });
    artRects.forEach(({ art, owner }) => {
      let progress = clamp(
        (viewport - owner.top) / Math.max(1, owner.height + viewport),
      );
      ...
      art.units.forEach((unit) => {
        ...
        const t = smooth(clamp((position - index) * 1.15 - unit.index * 0.025));
        const a = unit.poses[index],
          b = unit.poses[next];
        unit.node.style.transform = enabled
          ? "translate(" +
            mix(a[0], b[0], t).toFixed(2) +
            "px," +
            mix(a[1], b[1], t).toFixed(2) +
            "px) scale(" +
            mix(a[2], b[2], t).toFixed(3) +
            ")"
          : "";
        unit.pointer.style.transform =
          enabled && finePointer.matches
            ? "translate(" +
              (art.x * 8 * unit.depth).toFixed(2) +
              "px," +
              (art.y * 8 * unit.depth).toFixed(2) +
              "px)"
            : "";
      });
    });
  ...
  function scrollToPosition(top, onComplete, instant = false) {
    if (lenis && enabled && !instant) {
      ...
      // An instant history seek must also cancel any in-flight Lenis tween.
  ...
  function navigate(
    target,
    { push = true, instant = false, focus = true } = {},
  ) {
    ...
    let top =
      ...
      16;
    if (story?.pinned)
      top = story.start + (story.stages.indexOf(stage) + 0.4) * story.step;
    ...
    if (push && target.id) {
      ...
      if (location.hash !== hash) {
        try {
          ...
          /* Scrolling remains available when local-file history is restricted. */
        }
      }
    }
    scrollToPosition(
      clamp(top, 0, Math.max(0, root.scrollHeight - innerHeight)),
      () => {
  ...
  function applyMotion(restore = true) {
    ...
    if (disposed || pageSuspended || document.hidden) return;
    if (enabled && !lenis && typeof Lenis === "function") {
      lenis = new Lenis({
        lerp: 0.14,
        smoothWheel: true,
        syncTouch: false,
  ...
  stories.forEach((story) =>
    story.buttons.forEach((button, index) =>
      listen(button, "click", () => {
        ...
        const ownTicket = ++ticket;
        scrollToPosition(story.start + (index + 0.4) * story.step, () => {
          if (ownTicket === ticket) {
            paint();
  ...
  listen(document, "keydown", (event) => {
    ...
    if (event.key === " " && target.closest('button,summary,[role="button"]'))
      return;
    ticket++;
  ...
  listen(document, "pointerdown", () => cancelMotion(heroMotion), {
    passive: true,
  });
  ...
  function scheduleLayout() {
    ...
    resizeTimer = setTimeout(() => {
      ...
      if (
        layoutSize &&
        Math.abs(current.width - layoutSize.width) < 0.5 &&
        Math.abs(current.height - layoutSize.height) < 0.5 &&
        Math.abs(current.header - layoutSize.header) < 0.5
      )
        return;
      layout(true);
    }, 120);
  ...
  if (window.visualViewport)
    listen(window.visualViewport, "resize", requestPaint, { passive: true });
  if (typeof ResizeObserver === "function") {
    headerObserver = new ResizeObserver(() => {
      if (
        ready &&
        Math.abs(header.getBoundingClientRect().height - headerHeight) > 0.5
      )
        scheduleLayout();
    });
    ...
    sceneObserver = new IntersectionObserver(
      (entries) =>
        entries.forEach((entry) => {
          const art = arts.find((item) => item.element === entry.target);
          if (!art) return;
          art.visible = entry.isIntersecting;
          art.element.dataset.visible = String(entry.isIntersecting);
          requestPaint();
        }),
      { rootMargin: "120px" },
    );
  ...
  if (toolkit) {
    ...
    function selectTools(key, animate = false) {
      ...
      const direction =
        groups.indexOf(group) >= groups.indexOf(selectedTools) ? 1 : -1;
      selectedTools = group;
      ...
      filters.forEach((button) =>
        button.setAttribute(
          "aria-pressed",
          String(button.dataset.skillFilter === key),
        ),
      );
      const tiles = [...group.querySelectorAll(".skill-tile")];
      status.textContent =
        group.querySelector("h4").textContent + " / " + tiles.length + " tools";
      if (animate) {
        requestPaint();
        animateMotion(
          group,
          [
            {
              opacity: 0.85,
              transform:
                "translateX(" + direction * motion.skillsOffset + "px)",
            },
    ...
    toolkit.querySelector(".skill-filters").hidden = false;
    status.hidden = false;
    filters.forEach((button) =>
      listen(button, "click", () =>
        selectTools(button.dataset.skillFilter, true),
      ),
    );
  ...
  return function disposePortfolio() {
    if (disposed) return;
    disposed = true;
    ...
    listenerRemovers.splice(0).forEach((remove) => remove());
    originalContents.forEach(([element, children]) =>
      element.replaceChildren(...children),
    );
    originalAttributes.forEach(([element, attributes]) => {
